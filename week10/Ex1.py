...
with open("/Users/gihyeonkim/Desktop/school/2025_1/빅데이터분석실습/수업자료/파이썬 Beginner 소스/CSV/singer1.csv", 'r', encoding="cp949") as inFp:
    ...
    header = next(csvFile)  # 열 제목 읽어와서 리스트로 저장
    csvList.append(header)

    for row in csvFile: # 파일의 순수 데이터 행을 읽어와 리스트에 저장
        csvList.append(row)
    ...
    # 끝에서 두 번째 열의 값이 167 이상이면 모든 열의 배경색 '노랑'으로 설정
    idx = -2
    # csvList의 데이터를 workSheet 리스트로 옮기기
    for i in range (0, rowCnt):
        for j in range (0, colCnt):
            if csvList[i][idx].isnumeric(): # 끝에서 두 번째 열이 수치데이터인가
